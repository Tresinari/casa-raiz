...
export default async function ProdutoPage({ params }: { params: { slug: string } }) {
  ...
  const precoPix = Math.round(produto.preco * 0.95)
    ...
    <>
      ...
      <main className="max-w-5xl mx-auto px-4 py-12">
        ...
        <div className="grid md:grid-cols-2 gap-10">
              ...
              <div className="grid grid-cols-4 gap-2">
                {produto.imagens.map((url: string, i: number) => (
                  <div key={i} className="aspect-square rounded overflow-hidden border border-linen">
                    <img src={url} alt={`${produto.nome} ${i + 1}`} className="w-full h-full object-cover" />
                  </div>
                ))}
              </div>
            )}
          ...
          {/* INFORMAÇÕES */}
          <div>
            <p className="text-[11px] tracking-widest uppercase text-text-light mb-2">{produto.categoria}</p>
            ...
            <div className="mb-1">
              ...
              <span className="text-2xl font-medium text-bark">{formatarPreco(produto.preco)}</span>
            </div>
            <p className="text-sm text-forest-mid font-medium mb-6">
              ⚡ {formatarPreco(precoPix)} no Pix (5% off)
            </p>

            {/* Botão — Client Component separado (precisa do useCarrinho) */}
            ...
            {produto.estoque === 0 && (
              <p className="mt-4 text-xs text-red-500 font-medium">Produto esgotado</p>
            )}
          </div>
        </div>
      </main>
